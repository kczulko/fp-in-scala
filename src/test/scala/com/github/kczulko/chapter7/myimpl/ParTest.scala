...
class ParTest extends FlatSpec with Matchers {
  ...
  "map2" should "respect termination unit assigned to the tasks" in executionContext { implicit es =>
  ...
  it should s"throw ${classOf[CancellationException]} when requested timeout is too small" in executionContext { es =>
  ...
  "parFilter" should "hold elements that passes given predicate" in executionContext { es =>
    val future = Par.parFilter(List[Int](1, 2, 3, 4, 5, 6, 7, 8))(_ > 5)(es)
    future.get() should contain theSameElementsInOrderAs List(6,7,8)
  }
